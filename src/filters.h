...
std::vector< std::vector<double> > preProcessTW(std::vector<double> Bbdp, std::vector<double> Abdp, std::vector<double> Blp, std::vector<double> Alp, std::vector<double> MVC, std::vector< std::vector<double> > timeWindow, std::vector< std::vector<double> > timeWindow_prev){
	...
	*/
	...
	/* We apply the filter to the concatenation of the two time windows, and then only
	 * keep the part corresponding to the time window of interest. Thus we avoid the errors
	 * due to the zero padding in the computation of the convolution.
	 */
	std::vector<std::vector<double>> timeWindowFusion = std::vector<std::vector<double>>(timeWindow.size(), std::vector<double>(timeWindow[0].size() + timeWindow_prev[0].size(), 0.0));
	for(size_t i(0); i < timeWindow.size(); ++i) {

		for(size_t j(0); j < timeWindow_prev[0].size(); ++j) {
			timeWindowFusion[i][j] = timeWindow_prev[i][j];
		}

		for(size_t j(timeWindow_prev[0].size()); j < timeWindowFusion[0].size(); ++j) {
			timeWindowFusion[i][j] = timeWindow_prev[i][j - timeWindow_prev[0].size()];
		}

	}

	std::vector< std::vector<double> > filteredTWFusion;


	for (int i = 0; i < (int)timeWindowFusion.size(); i++){
		filteredTWFusion.push_back(preProcessSignal(Bbdp, Abdp, timeWindowFusion[i], Blp, Alp, MVC[i]));
	}


	std::vector< std::vector<double> > filteredTW(filteredTWFusion.size(), std::vector<double>(timeWindow[0].size(), 0.0));
	for (size_t i(0); i < filteredTWFusion.size(); ++i) {
		for (size_t j(0); j < timeWindow[0].size(); ++j) {
			filteredTW[i][j] = filteredTWFusion[i][j + timeWindow_prev[0].size()];
		}
	}
